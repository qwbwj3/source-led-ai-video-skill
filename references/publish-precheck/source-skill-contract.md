# Original yuwen-publish-precheck contract snapshot

## Portable embedding adapter

This snapshot preserves the original judgment rules, hard boundaries, evidence standard, candidate-by-candidate review discipline, and output contract. In this video Skill, the outer workflow adapts the original Skill's command entry points, interactive profile setup, and mutable-memory behavior:

- treat `profile.md`, `my-rules.md`, and `expressions.md` in this directory as pinned read-only inputs;
- do not create or modify an installation-level `data/` directory;
- do not run first-use onboarding or ask profile questions already fixed by the video project contract;
- save each review and approval under the current video's `review/` directory instead of `data/history/`;
- route any future rule-learning request back to the separately installed `yuwen-publish-precheck` Skill; never mutate this release snapshot.
- use `review_gate.py prepare/scan/approve --config <project.json>` as the only embedded command sequence. The original `scripts/scan.py` command and mutable `data/` paths shown later in this preserved contract do not apply inside this portable video Skill; `review_gate.py scan` binds scope to the prepared config and reads the pinned rules in this directory.

These embedding rules have priority for command orchestration, file locations, onboarding, and persistence. They do not weaken any content judgment or safety rule below.
